...
class TransferModel(BaseModel):
        ...
        if self.isTrain:
            ...
            if opt.L1_type == 'origin':
                ...
                self.criterionL1 = L1_plus_perceptualLoss(
                    opt.lambda_A, opt.lambda_B, opt.perceptual_layers, self.gpu_ids, opt.percep_is_l1)
            else:
            ...
            if opt.use_cxloss:
                ...
                if torch.cuda.is_available():
                    self.CX_loss.cuda()
                self.vgg = VGG()
                self.vgg.load_state_dict(torch.load(os.path.abspath(
                    os.path.dirname(opt.dataroot)) + '/vgg_conv.pth'))
            ...
            self.optimizer_G = torch.optim.Adam(filter(
                lambda p: p.requires_grad, self.netG.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))

            if opt.with_D_PB:
                self.optimizer_D_PB = torch.optim.Adam(
                    self.netD_PB.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            if opt.with_D_PP:
                self.optimizer_D_PP = torch.optim.Adam(
                    self.netD_PP.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def backward_G(self):
        if self.opt.with_D_PB:
            pred_fake_PB = self.netD_PB(
                torch.cat((self.fake_p2, self.input_BP2), 1))
            self.loss_G_GAN_PB = self.criterionGAN(pred_fake_PB, True)

        if self.opt.with_D_PP:
            pred_fake_PP = self.netD_PP(
                torch.cat((self.fake_p2, self.input_P1), 1))
        ...
        if self.opt.L1_type == 'l1_plus_perL1':
            ...
            self.loss_G_L1 = self.criterionL1(
                self.fake_p2, self.input_P2) * self.opt.lambda_A
    ...
    def backward_D_PB(self):
        ...
        fake_PB = self.fake_PB_pool.query(
            torch.cat((self.fake_p2, self.input_BP2), 1).data)
